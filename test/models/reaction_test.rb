...
class ReactionTest < Minitest::Test
  def test_basic_reaction
    reaction = Slk::Models::Reaction.new(name: 'thumbsup', count: 5)
  ...
  def test_reaction_with_users
    users = %w[U123 U456]
    reaction = Slk::Models::Reaction.new(name: 'heart', count: 2, users: users)
    assert_equal users, reaction.users
  end

  def test_emoji_code
    reaction = Slk::Models::Reaction.new(name: 'fire')
    assert_equal ':fire:', reaction.emoji_code
  end

  def test_to_s
    reaction = Slk::Models::Reaction.new(name: 'star', count: 3)
  ...
  def test_from_api
    ...
    reaction = Slk::Models::Reaction.from_api(data)
  ...
  def test_values_are_frozen
    reaction = Slk::Models::Reaction.new(name: 'smile', count: 1, users: %w[U123])
    assert reaction.name.frozen?
    assert reaction.users.frozen?
  end

  def test_default_count_is_zero
    reaction = Slk::Models::Reaction.new(name: 'test')
    assert_equal 0, reaction.count
  end

  def test_negative_count_normalized_to_zero
    reaction = Slk::Models::Reaction.new(name: 'test', count: -5)
    assert_equal 0, reaction.count
  end
  ...
  def test_with_timestamps
    reaction = Slk::Models::Reaction.new(name: 'star', count: 2, users: %w[U123 U456])
  ...
  def test_timestamps_predicate_with_nil
    reaction = Slk::Models::Reaction.new(name: 'wave', count: 1)
    assert_equal false, reaction.timestamps?
  end

  def test_timestamps_predicate_with_empty_hash
    reaction = Slk::Models::Reaction.new(name: 'wave', count: 1, timestamps: {})
    assert_equal false, reaction.timestamps?
  end

  def test_timestamps_predicate_with_timestamps
    timestamps = { 'U123' => '1767996268.000000' }
    reaction = Slk::Models::Reaction.new(name: 'wave', count: 1, timestamps: timestamps)
    assert_equal true, reaction.timestamps?
  end

  def test_timestamp_for_user
    timestamps = { 'U123' => '1767996268.000000', 'U456' => '1767996300.000000' }
    reaction = Slk::Models::Reaction.new(name: 'fire', count: 2, timestamps: timestamps)
  ...
  def test_timestamp_for_without_timestamps
    reaction = Slk::Models::Reaction.new(name: 'fire', count: 1)
    assert_nil reaction.timestamp_for('U123')
  end

  def test_timestamps_are_frozen
    timestamps = { 'U123' => '1767996268.000000' }
    reaction = Slk::Models::Reaction.new(name: 'heart', count: 1, timestamps: timestamps)
