...
class PresetTest < Minitest::Test
  def test_basic_preset
    preset = Slk::Models::Preset.new(name: 'lunch', text: 'Out to lunch', emoji: ':fork_and_knife:')
  ...
  def test_from_hash
    ...
    preset = Slk::Models::Preset.from_hash('wfh', data)
  ...
  def test_from_hash_with_missing_fields
    preset = Slk::Models::Preset.from_hash('empty', {})
  ...
  def test_to_s_basic
    preset = Slk::Models::Preset.new(name: 'lunch', text: 'Lunch', emoji: ':fork_and_knife:')
    assert_equal 'lunch: :fork_and_knife: "Lunch"', preset.to_s
  end

  def test_to_s_with_duration
    preset = Slk::Models::Preset.new(name: 'meeting', text: 'Meeting', duration: '1h')
  ...
  def test_values_are_frozen
    preset = Slk::Models::Preset.new(name: 'test', text: 'Testing')
  ...
  def test_duration_value_parses_duration
    preset = Slk::Models::Preset.new(name: 'test', duration: '2h30m')
    duration = preset.duration_value
    refute_nil duration
    assert_kind_of Slk::Models::Duration, duration
  end

  # Validation tests
  def test_raises_when_name_empty
    error = assert_raises(ArgumentError) do
      Slk::Models::Preset.new(name: '')
    end
  ...
  def test_strips_whitespace_from_name
    preset = Slk::Models::Preset.new(name: '  lunch  ')
    assert_equal 'lunch', preset.name
  end

  def test_raises_when_duration_invalid
    error = assert_raises(ArgumentError) do
      Slk::Models::Preset.new(name: 'test', duration: 'invalid')
    end
  ...
  def test_raises_when_duration_has_duplicate_units
    error = assert_raises(ArgumentError) do
      Slk::Models::Preset.new(name: 'test', duration: '1h1h')
    end
    assert_match(/duplicate/i, error.message)
  end

  def test_allows_zero_duration
    preset = Slk::Models::Preset.new(name: 'test', duration: '0')
    assert_equal '0', preset.duration
  end

  def test_allows_empty_duration
    preset = Slk::Models::Preset.new(name: 'test', duration: '')
