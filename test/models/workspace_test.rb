...
class WorkspaceTest < Minitest::Test
  def test_creates_with_required_fields
    ws = Slk::Models::Workspace.new(name: 'test', token: 'xoxb-123')
  ...
  def test_creates_with_cookie
    ws = Slk::Models::Workspace.new(name: 'test', token: 'xoxc-123', cookie: 'abc')
  ...
  def test_xoxp_detection
    ws = Slk::Models::Workspace.new(name: 'test', token: 'xoxp-123')
  ...
  def test_to_s_returns_name
    ws = Slk::Models::Workspace.new(name: 'myworkspace', token: 'xoxb-123')
  ...
  def test_raises_on_empty_name
    error = assert_raises(ArgumentError) do
      Slk::Models::Workspace.new(name: '', token: 'xoxb-123')
    end
  ...
  def test_raises_on_name_with_path_separators
    error = assert_raises(ArgumentError) do
      Slk::Models::Workspace.new(name: '../etc', token: 'xoxb-123')
    end
  ...
  def test_raises_on_invalid_token_format
    error = assert_raises(ArgumentError) do
      Slk::Models::Workspace.new(name: 'test', token: 'invalid-token')
    end
  ...
  def test_raises_on_xoxc_without_cookie
    error = assert_raises(ArgumentError) do
      Slk::Models::Workspace.new(name: 'test', token: 'xoxc-123')
    end
  ...
  def test_raises_on_xoxc_with_empty_cookie
    error = assert_raises(ArgumentError) do
      Slk::Models::Workspace.new(name: 'test', token: 'xoxc-123', cookie: '')
    end
    assert_match(/xoxc tokens require a cookie/, error.message)
  end
  ...
  def test_raises_on_cookie_with_newline
    error = assert_raises(ArgumentError) do
      Slk::Models::Workspace.new(name: 'test', token: 'xoxc-123', cookie: "value\ninjected")
    end
  ...
  def test_raises_on_cookie_with_carriage_return
    error = assert_raises(ArgumentError) do
      Slk::Models::Workspace.new(name: 'test', token: 'xoxc-123', cookie: "value\rinjected")
    end
    assert_match(/cannot contain newlines/, error.message)
  end
end
